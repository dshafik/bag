...
- `InputPipeline` for handling input and constructing the Bag object (called by `Bag::from()`)
- `ValidationPipeline` for validating without constructing the Bag object (called by `Bag::validate()`)
- `OutputPipeline` for handling output (called by `Bag::toArray()`, `Bag::jsonSerialize()`/`Bag::toJson()`, and `Bag::unwrapped()`)
- `OutputCollectionPipeline` for handling collection output (called by `Bag\Collection::toArray()` and `Bag\Collection::jsonSerialize()`/`Bag\Collection::toJson()`)
